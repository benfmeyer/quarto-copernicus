...
author:
  - given_name: Robert 
    surname: Zimmerman
    affiliation: "1, *"
    email: robert.zimmerman@newport.com
    corresponding: true
  - given_name: Diane
    surname: Young
    affiliation: 2
    email: diane.young@contra.com
  - given_name: Charley
    surname: Crockett
    affiliation: "1, *"
  - given_name: Townes
...
# Two authors: Robert Zimmerman (robert.zimmerman@newport.com) and Diane Young (diane.young@contra.com)
# Three authors or more: Robert Zimmerman (robert.zimmerman@newport.com), Diane Young (diane.young@contra.com), and Charley Crockett (cc@san-benito.tx)
# If the following line is uncommented, the "corresponding: true" above are ignored
#correspongdingauthors: Robert Zimmerman (robert.zimmerman@newport.com) and Diane Young (diane.young@contra.com)
# If an author is deceased, please mark the respective author name(s) with a dagger '†' and add a further affiliation; put the decease date in the 'address' field", see 'Townes Van Zandt' in template.
...
affiliation:
  - code: 1
    address: Institute for Philosophy and Literature, University of Minnesota, Duluth, MN, USA
  - code: 2
    address: Institute for Comma Science, University of Oxford, Oxford, England
  - code: 3
    address: Department of Honky Tonk, University of Hard Times, Waco, TX, USA 
  - code: "†"
    address: deceased, 1 January 1997
  - code: "*"
...
running:
  title: Quarto Template for Copernicus
...
authorcontribution: |
  RZ wrote the manuscript. DY changed her mind. CC contributed a wealth of life experience. TVZ told stories.
disclaimer: |
  We like Copernicus.
acknowledgements: |
  Based entirely on the original work by the rticles developers and simply adapted to work with Quarto. 
appendix: |
...
This is a port of the `rticles::copernicus_article()` to Quarto. Original work by the `rticles` developers/contributors. Quarto implementation by Benjamin F. Meyer.  All sample text is taken directly from the `rticles` template.

You can change the name of the section if necessary using `\introduction[modified heading]`.

- The `journal` you are submitting to using the official abbreviation. You can use the function to search the existing journals.

- The `pdf-engine` to be used to render the document. At the moment the Quarto default `xelatex` throws errors but `pdflatex` works
